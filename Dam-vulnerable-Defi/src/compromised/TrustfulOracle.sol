...
contract TrustfulOracle is AccessControlEnumerable {
    ...
    constructor(address[] memory sources, bool enableInitialization) {
        ...
        for (uint256 i = 0; i < sources.length; ) {
            //mapping to trusted_source_role  and source act as oracle
            unchecked {
    ...
    function setupInitialPrices(
        ...
        renounceRole(INITIALIZER_ROLE, msg.sender); //no one will call this function
    }

    //@audit this is used to change the price of a symbol by a trusted source
    function postPrice(
    ...
    // this is vulnerable to change the price of a symbol by a trusted source but this is private function u never call this function
    function _setPrice(
        address source,
        string memory symbol,
        uint256 newPrice
